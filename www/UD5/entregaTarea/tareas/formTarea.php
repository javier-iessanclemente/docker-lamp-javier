...
<div class="mb-3">
    ...
    <select class="form-select" id="id_usuario" name="id_usuario" required>
        <option value="" <?php echo (isset($usuarioTarea) || !esAdmin()) ? '' : 'selected' ?> disabled>Seleccione el usuario</option>
        <?php
            ...
            if(esAdmin()) {
                ...
                $usuarios= [buscaUsuario($usuario)];
            }
            foreach ($usuarios as $usuario) { ?>
                <option value="<?php echo ($usuario->getId()); ?>" <?php echo ((isset($usuarioTarea) && $usuarioTarea->getId() == $usuario->getId()) || !esAdmin()) ? 'selected' : '' ?>>
